...
def init_schema(conn: sqlite3.Connection) -> None:
    ...
    # Migrate existing DBs that lack the instructions column
    try:
        conn.execute("ALTER TABLE users ADD COLUMN instructions TEXT;")
        conn.commit()
...
# ============================================
# Reply Count & Coupon Code Functions
# ============================================

def get_user_reply_info(conn: sqlite3.Connection, wa_id: str) -> dict:
    """Get user's reply count, max replies, and unlimited status."""
    cur = conn.execute(
...
def can_user_reply(conn: sqlite3.Connection, wa_id: str) -> bool:
    """Check if user can still reply (within limits or unlimited)."""
...
def increment_reply_count(conn: sqlite3.Connection, wa_id: str) -> None:
    """Increment user's reply count by 1."""
    conn.execute(
...
def apply_coupon_code(conn: sqlite3.Connection, wa_id: str, code: str) -> bool:
    """Apply coupon code to user. Returns True if valid and applied."""
